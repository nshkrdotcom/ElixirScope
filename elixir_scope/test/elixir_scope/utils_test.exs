defmodule ElixirScope.UtilsTest do
  ...
  describe "generate_event_id/0" do
    test "returns a string (checking as binary and UUID format)" do # Test name updated
      event_id = Utils.generate_event_id()
      assert is_binary(event_id) # Check if it's a binary
      assert Regex.match?(~r/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i, event_id) # Check UUID v4 format
    end
  ...
  describe "monotonic_time_ns/0" do
    test "returns an integer" do
      ...
      assert is_integer(time_ns) # Changed to is_integer without Kernel prefix
    end
